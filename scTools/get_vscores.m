function [v_scores, FF_gene, mu_gene, CV_eff, CV_input, a, b, ax_CV, ax_FF] = get_vscores(X_norm, tot_counts, varargin)
% Usage: [v_scores, FF_gene, mu_gene, CV_eff, CV_input, a, b, ax_CV, ax_FF] = get_vscores(X_norm, tot_counts, varargin)
% 
...
%
% ax_CV             : Handle to figure, if requested
%
% ax_FF             : Handle to figure, if requested
%
...
% Plot:
p=scatter(mu_gene, CV_gene, 5, plt_v, 'o', 'Filled'); noLegend(p);
colormap(jet(100))
...
plot(xTh,1./xTh.^0.5,'-','color',[0 0 0],'linewidth',1)
plot(xTh,yTh,'k--','linewidth',1)
set(gca,'fontsize',15)
xlabel('Avg. Reads per Cell')
ylabel('Coefficient of Variation')
set(gca,'xlim',[1e-3 x_max],'xtick',10.^(-4:4))
...
% Plot:
p=scatter(mu_gene, FF_gene, 5, plt_v, 'o', 'Filled'); noLegend(p);
colormap(jet(100))
set(gca,'yscale','log','xscale','log');
axis square
cbar_hndl = colorbar;
