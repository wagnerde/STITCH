function [ydata, cost] = tsne(X, labels, no_dims, initial_dims, perplexity, iterations)
...
% University of California, San Diego
    ...
    if ~exist('no_dims', 'var') || isempty(no_dims)
     ...
     if ~exist('initial_dims', 'var') || isempty(initial_dims)
        %initial_dims = min(50, size(X, 2));              % original
        initial_dims = min(max(50, no_dims), size(X, 2)); % DEW modified in case no_dims is >50
     end
    ...
    if ~initial_solution
        ...
        % M is eigenvector matrix: each column is an eigenvector, rows are genes
        % lambda is the matrix of eigenvalues
        [M, lambda] = eig(C);
    ...
    % Run t-SNE
    if initial_solution
        [ydata, cost] = tsne_p(P, labels, ydata, iterations);
    else
        [ydata, cost] = tsne_p(P, labels, no_dims, iterations);
    end
